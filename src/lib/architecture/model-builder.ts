...
function inferNodeType(symbol: SymbolRow): ArchNodeType {
  ...
  if (symbol.symbolType === "class") {
    ...
    if (fileLower.includes("service")) return "service";
    if (fileLower.includes("db") || fileLower.includes("database") || fileLower.includes("repository") || fileLower.includes("model")) return "database";
    return "service";
  }
